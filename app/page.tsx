...
const ImportAlternatives = dynamic(
    () =>
        import("./components/features/ImportAlternatives").then(
            (m) => m.ImportAlternatives
        ),
    {
        ssr: false,
        loading: () => (
...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const workspaceRaw = window.localStorage.getItem("spk-workspace");
        if (workspaceRaw) {
            try {
                const parsed = JSON.parse(workspaceRaw) as WorkspaceState;
                if (parsed && parsed.alternatives && parsed.criteria) {
                    setWorkspace(parsed);
                }
            } catch {
                // ignore
    ...
    useEffect(() => {
        if (typeof window === "undefined") return;
        window.localStorage.setItem("spk-workspace", JSON.stringify(workspace));
    }, [workspace]);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!response.ok) {
                    setNotification(
                        "Gagal memuat riwayat dari server. Pastikan koneksi/database siap."
                    );
                    return;
                }
    ...
    const resetAlternativeForm = () =>
        setAlternativeForm(defaultAlternativeForm(""));
    ...
    const ensureCustomWeights = (
        criteria: Criteria[],
        current: WorkspaceState["customWeights"]
    ) => {
        ...
        if (currentUser) {
            void (async () => {
                try {
                    ...
                    if (!resp.ok) {
                        setNotification(
                            "Riwayat gagal disimpan ke server. Cek koneksi/database."
                        );
                    } else {
                        setNotification(
                            `Perhitungan "${name}" disimpan ke riwayat (server & lokal)`
                        );
                    }
                } catch {
                    setNotification(
            ...
            setNotification(
                `Perhitungan "${name}" disimpan di perangkat ini (belum login).`
            );
        }
    };
    ...
    const handleLoadHistory = (id: string) => {
        ...
        const hydratedWorkspace: WorkspaceState = {
            ...
            customWeights: ensureCustomWeights(
                item.workspace.criteria ?? [],
                item.workspace.customWeights ?? {}
            )
        };
        setWorkspace(hydratedWorkspace);
        setAlternativeForm(
            defaultAlternativeForm(`A${item.workspace.alternatives.length + 1}`)
        );
        setCriteriaForm(
            defaultCriteriaForm(`C${item.workspace.criteria.length + 1}`)
        );
        setAhpOverrideApproved(false);
        setTopsisTab("decision");
        setMainTab("dashboard");
    ...
    const handleAlternativeSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        setWorkspace((prev) => {
            ...
            const alternatives = isEdit
                ? prev.alternatives.map((item) =>
                      item.id === alternative.id ? alternative : item
                  )
                : [...prev.alternatives, alternative];

            const syncedScores = syncScoresStructure(
                alternatives,
                prev.criteria,
                prev.scores
            );
        ...
        setNotification(
            editing ? "Alternatif diperbarui" : "Alternatif ditambahkan"
        );
    };

    const handleAlternativeDelete = (id: string) => {
        setWorkspace((prev) => {
            const alternatives = prev.alternatives.filter(
                (item) => item.id !== id
            );
    ...
    const handleImportAlternatives = (importedData: {
        ...
        setWorkspace((prev) => {
            const newAlternatives: Alternative[] =
                importedData.alternatives.map((item, index) => ({
                    id: item.id || crypto.randomUUID(),
                    code:
                        item.code || `A${prev.alternatives.length + index + 1}`,
                    name:
                        item.name ||
                        `Alternatif ${prev.alternatives.length + index + 1}`,
            ...
            const mergedScores = importedData.scores
                ? { ...prev.scores, ...importedData.scores }
                : prev.scores;
            const scoresToUse = syncScoresStructure(
                combinedAlternatives,
                criteriaToUse,
                mergedScores
            );
            const nextCustomWeights = ensureCustomWeights(
                criteriaToUse,
                prev.customWeights
            );
            ...
            return {
                ...
                scores: scoresToUse,
                pairwiseMatrix: importedData.criteria
                    ? sanitizePairwiseMatrix(criteriaToUse, {})
                    : prev.pairwiseMatrix,
        ...
        setNotification(
            `${importedData.alternatives.length} alternatif berhasil diimport`
        );
    };
    ...
    const handleCriteriaSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        setWorkspace((prev) => {
            ...
            const criteria: Criteria = {
                ...
                position: isEdit
                    ? prev.criteria.find((item) => item.id === criteriaForm.id)
                          ?.position
            ...
            const reindexedCriteria = criteriaCollection.map((item, index) => ({
                ...item,
                position: index
            }));
            const scores = syncScoresStructure(
                prev.alternatives,
                reindexedCriteria,
                prev.scores
            );
            const pairwiseMatrix = sanitizePairwiseMatrix(
                reindexedCriteria,
                prev.pairwiseMatrix
            );
            const customWeights = ensureCustomWeights(
                reindexedCriteria,
                prev.customWeights
            );
        ...
        resetCriteriaForm();
        setNotification(
            editing ? "Kriteria diperbarui" : "Kriteria ditambahkan"
        );
        setAhpOverrideApproved(false);
    };
    ...
    const handleCriteriaDelete = (id: string) => {
        setWorkspace((prev) => {
            ...
            const reindexed = criteria.map((item, index) => ({
                ...item,
                position: index
            }));
            const scores = removeCriteriaFromScores(prev.scores, id);
            const pairwiseMatrix = removeCriteriaFromMatrix(
                prev.pairwiseMatrix,
                id
            );
            const customWeights = sanitizeCustomWeights(
                reindexed,
                prev.customWeights
            );
    ...
    const handlePairwiseChange = (
        ...
        setWorkspace((prev) => ({
            ...prev,
            pairwiseMatrix: updatePairwiseValue(
                prev.pairwiseMatrix,
                rowId,
                colId,
                value
    ...
    const handleWeightingModeChange = (mode: WeightingMode) => {
        setWorkspace((prev) => ({
            ...
            customWeights:
                mode === "CUSTOM"
                    ? ensureCustomWeights(prev.criteria, prev.customWeights)
                    : prev.customWeights,
            topsisResults: undefined,
            topsisDetail: undefined
        }));
    ...
    const handleCalculateAhp = () => {
        ...
        try {
            const result = calculateAhpResult(
                workspace.criteria,
                workspace.pairwiseMatrix
            );
            setWorkspace((prev) => ({
                ...prev,
                ahpResult: result,
    ...
    const handleCalculateTopsis = () => {
        ...
        if (workspace.weightingMode === "CUSTOM") {
            const customReady = areCustomWeightsReady(
                workspace.criteria,
                workspace.customWeights
            );
            if (!customReady) {
                setNotification(
                    "Lengkapi bobot custom untuk semua kriteria (isi angka > 0)"
                );
                return;
            }
            activeWeights = sanitizeCustomWeights(
                workspace.criteria,
                workspace.customWeights
            );
        } else {
            ...
            if (!workspace.ahpResult.isConsistent && !ahpOverrideApproved) {
                setNotification(
                    "Konfirmasi penggunaan bobot dengan CR > 0.1 terlebih dahulu"
                );
                return;
            }
                ...
                <Card className="w-full max-w-sm shadow-lg">
                    <CardContent className="p-6">
                        ...
                        <h1 className="mb-4 text-xl font-semibold">
                            Masuk ke Dashboard SPK
                        </h1>
                        <p className="mb-4 text-xs text-muted-foreground">
                            Gunakan akun sementara{" "}
        ...
        <div className="min-h-screen bg-background text-foreground font-sans">
            ...
            <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div>
                    ...
                    {mainTab === "results" && (
                        <ResultsView workspace={workspace} />
                    )}
                </div>
            </main>
            ...
            >
                <p className="text-sm text-muted-foreground">
                    Pastikan Anda telah menyimpan pekerjaan Anda saat ini jika
                    masih diperlukan.
                </p>
            </Modal>
                ...
                <form onSubmit={handleSaveHistorySubmit} className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                setHistoryNameInput(e.target.value)
                            }
