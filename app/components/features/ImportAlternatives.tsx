...
export const ImportAlternatives = ({ isOpen, onClose, onImport }: ImportAlternativesProps) => {
        ...
        {step === "format" && (
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-4 shadow-sm">
              <p className="text-sm text-muted-foreground">Pilih sumber data yang akan digunakan.</p>
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
                {[
                  { id: "json", title: "JSON", desc: "Tempel array of objects", disabled: false },
                  { id: "excel", title: "Excel / CSV", desc: "Upload + pilih sheet", disabled: true },
                  { id: "sql", title: "SQL", desc: "Koneksi DB + query SELECT", disabled: true },
                ].map((opt) => (
                  <button
                    key={opt.id}
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-white/80 p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold">Tempel data {format.toUpperCase()}</p>
              ...
              {format !== "json" && (
                <div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                  Dukungan {format.toUpperCase()} akan ditambahkan. Gunakan JSON terlebih dahulu untuk melanjutkan alur import.
                </div>
              )}

              <div className="mt-3 space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-medium text-muted-foreground">Tempel JSON di sini</label>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-xs"
                    onClick={() => setJsonText(SAMPLE_JSON.trim())}
                  >
                    Gunakan contoh data
                  </Button>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between text-xs text-muted-foreground">
                  <span>Tip: Nama kolom bebas, akan di-mapping pada langkah berikutnya.</span>
                  <span>{jsonText.length.toLocaleString()} karakter</span>
                </div>
              </div>
            </div>
        ...
        {step === "mapping" && rawData.length > 0 && (
          <div className="space-y-4">
            <div className="space-y-3">
              <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                <div className="flex items-center justify-between text-xs">
                  <div>
                    <div className="font-semibold">{rawData.length} baris</div>
                    <div className="text-muted-foreground">{availableFields.length} kolom</div>
                  </div>
                  <div className="text-muted-foreground">
                    Preview {Math.min(previewRows.length, MAX_PREVIEW_ROWS)} baris pertama.
                  </div>
                </div>
                    ...
                    <thead className="bg-slate-50">
                      <tr>
                        {availableFields.map((field) => (
                          <th key={field} className="px-2 py-2 text-left font-semibold text-slate-700">
                            {field}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {previewRows.map((row, idx) => (
                        <tr key={idx} className="odd:bg-white even:bg-slate-50">
                          {availableFields.map((field) => (
                            <td key={field} className="px-2 py-1.5 text-[11px] text-slate-700">
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-semibold">Mapping Alternatif</h4>
                  <span className="text-[11px] text-muted-foreground">{rawData.length} baris</span>
                </div>
                ...
                <label className="text-xs font-medium text-muted-foreground">
                  ...
                  >
                    <option value="">-- Gunakan ID sebagai Nama --</option>
                    {availableFields.map((field) => (
                      <option key={field} value={field}>
                        {field}
                      </option>
              ...
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 shadow-sm space-y-2">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-emerald-900">Pilih Field sebagai Kriteria</h4>
                  <div className="text-[11px] text-emerald-700">
                    {selectedCriteriaCount} dipilih • Maks {MAX_CRITERIA}
                  </div>
                </div>
                  ...
                  {Object.entries(criteriaSelections).map(([field, config]) => (
                    <div
                      ...
                      className={`flex items-center gap-3 rounded border p-2 text-xs ${
                        config.selected ? "border-emerald-300 bg-white" : "border-emerald-100 bg-white/70"
                      }`}
                    >
                      <input
                        type="checkbox"
                        checked={config.selected}
                        disabled={!config.selected && criteriaLimitReached}
                        onChange={(e) => {
                          const nextSelected = e.target.checked;
                          setCriteriaSelections((prev) => {
                            const currentSelected = Object.values(prev).filter((item) => item.selected).length;
                            if (!prev[field]?.selected && nextSelected && currentSelected >= MAX_CRITERIA) {
                              return prev;
                            }
                            return { ...prev, [field]: { ...prev[field], selected: nextSelected } };
                          });
                        }}
                      ...
                      <select
                        ...
                        className="rounded border border-slate-200 px-2 py-1 text-xs disabled:bg-slate-100 disabled:text-slate-500"
                      >
                ...
                {selectedCriteriaCount === 0 && (
                  <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-[11px] text-amber-800">
                    Pilih minimal 1 kriteria numerik agar TOPSIS bisa dijalankan. Anda masih bisa menambah kriteria manual setelah import.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-lg border border-slate-200 p-3 bg-white/70 shadow-sm">
              <p className="text-xs font-medium text-muted-foreground mb-2">Preview cepat (baris pertama):</p>
              <div className="bg-slate-50 p-3 rounded-lg text-xs space-y-1">
                <div className="flex flex-wrap gap-3">
                  ...
                  <div>
                    ...
                    <span className="font-semibold">
                      {fieldMapping.nameField ? rawData[0][fieldMapping.nameField] || "-" : rawData[0][fieldMapping.codeField] || "-"}
                    </span>
                  </div>
                ...
                {selectedCriteriaCount > 0 && (
                  <div className="pt-2 mt-2 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-1">
                    {selectedCriteriaEntries.map(([field, config]) => (
                      <div key={field}>
                        <span className="text-muted-foreground">{config.name}:</span>{" "}
                        <span className="font-semibold">{rawData[0][field]}</span>{" "}
                        <span className="text-[11px] text-muted-foreground">({config.type})</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="flex justify-between">
              ...
              <Button onClick={handleImportExecute} disabled={!mappingReady}>
                Proses Import
              </Button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-emerald-900">Import selesai</p>
              ...
              <div className="mt-3 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                <div className="rounded-md bg-white/70 px-3 py-2 border border-emerald-100">
                  <div className="text-xs text-muted-foreground">Alternatif</div>
                  <div className="text-lg font-semibold text-emerald-900">
                    {summary.alternatives.toLocaleString()} data
                  </div>
                </div>
                <div className="rounded-md bg-white/70 px-3 py-2 border border-emerald-100">
                  <div className="text-xs text-muted-foreground">Kriteria aktif</div>
                  <div className="text-lg font-semibold text-emerald-900">
                    {summary.criteria} dipilih (maks {MAX_CRITERIA})
                  </div>
                </div>
                <div className="rounded-md bg-white/70 px-3 py-2 border border-emerald-100">
                  <div className="text-xs text-muted-foreground">Kolom sumber</div>
                  <div className="text-lg font-semibold text-emerald-900">
                ...
                <div className="rounded-md bg-white/70 px-3 py-2 border border-emerald-100 text-xs text-emerald-800 space-y-1">
                  <div>
                    Field ID: <span className="font-semibold text-emerald-900">{summary.codeField}</span>
                  </div>
                  <div>
                    Nama:{" "}
                  ...
                  <div>
                    Deskripsi:{" "}
                    <span className="font-semibold text-emerald-900">{summary.descField || "-"}</span>
                  </div>
                  <div>
                    Format: <span className="font-semibold text-emerald-900">{summary.format.toUpperCase()}</span>
                  </div>
                </div>
              </div>
            </div>
