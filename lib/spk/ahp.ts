...
export function calculateAhpResult(criteria: Criteria[], matrix: AhpComparisonMatrix): AhpResult {
  if (!criteria.length) {
  ...
  const columnSums = rawMatrix.reduce<number[]>(
    (sums, row) => {
  ...
  const lambdaMax = columnSums.reduce(
    (sum, col, index) => sum + col * weights[index],
    0,
  );
